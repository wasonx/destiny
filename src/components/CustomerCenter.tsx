...
import { PackageCheck, ReceiptText, RefreshCw, ShoppingBag, Sparkles } from 'lucide-react';
import { fetchValueState } from '../lib/customerAuth';
import { createOrder, createRefundRequest, CustomerOrder, CustomerProduct, listOrders, listProducts } from '../lib/customerCommerce';
...
export default function CustomerCenter() {
  ...
  async function loadCenter() {
    ...
    setMessage('');
    try {
      const [stateData, productData, orderData] = await Promise.all([
  ...
  async function buyProduct(product: CustomerProduct) {
    ...
    if (product.requires_shipping || product.product_type === 'physical_goods') {
      setMessage('实物商品需要收货地址，第一版请在小程序内维护地址后下单。');
      return;
    }
