...
| 上传版本 | `1.0.0` |
| 上传备注 | 甄好算1.0正式版：真实微信登录、知识库后台、图计算、图谱可视化、报告分层、商城、会员积分、罗盘、订单退款发货闭环 |
...
- 微信小程序审核：已通过。
- 本地完整验证：`npm run test:miniprogram`、`npm run test:server`、`npm run test:py`、`npm run lint`、`npm run build` 均通过。
- 小程序静态验证输出：`miniprogram_static_ok=17`、`json_files_ok=23`、`native_pages_ok=13`。
- 服务端测试输出：94 项全部通过。
- Python 验收输出：44 项全部通过。
- 线上入口：`https://www.goye.cc`、`https://www.goye.cc/admin`、`https://www.goye.cc/destiny-api/health` 均验证可用。
- 待用户在微信公众平台完成最终发布：`管理 > 版本管理 > 审核版本 > 发布`。
